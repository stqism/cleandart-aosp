...
static struct vreg *vreg_proximity_LEDA;
#endif
...
static int opt_i2c_read(u8 reg, u8 *val, unsigned int len )
{
 ...
 if (i >= GP2A_RETRY_COUNT) {
  pr_err("%s: retry over %d\n", __FUNCTION__, GP2A_RETRY_COUNT);
  return -EIO;
 }
#else
	err = i2c_transfer(opt_i2c_client->adapter, msg, 1);
...
//choh-sysfs : create a sysfs file "proximity_int" in the switch_cmd dir
static ssize_t proximity_int_show(struct device *dev, struct device_attribute *attr, char *buf)	
{
	int int_val = 0;

	int_val = (int)((proximity_value==1)? 1:2);

	printk("[%s] proxi_int = %x\n", __func__, int_val);
	
	return sprintf(buf,"%d\n",int_val);
}

static ssize_t proximity_int_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)		
{	
...
static DEVICE_ATTR(proximity_int, 0666, proximity_int_show, proximity_int_store);
//end of choh-sysfs
...
gprintk("[PROXIMITY] %s : \n",__func__);
	...
	printk(KERN_INFO "[PROXIMITY] AXIS_FLAG = %d, PROX_FLAG =%d \n", AXIS_FLAG, PROX_FLAG);
#endif
...
#if defined (CONFIG_MACH_GT2)
#if !defined (CONFIG_MACH_GT2R)	// do not use ldo 15 on GT2R
if( board_hw_revision < 0x4 )
{
		vreg_enable(vreg_proximity); // voltage 
}
#endif
	vreg_enable(vreg_proximity_LEDA); // voltage 
...
// do not use ldo 15 on GT2R and do not turn off LDO related with Proximity sensor
if( board_hw_revision < 0x4 )
{
			vreg_disable(vreg_proximity);
			vreg_disable(vreg_proximity_LEDA); // voltage 
}
else
	gprintk("gp2a_off GT2(%d)\n",type);
#else
	gprintk("gp2a_off GT2R(%d)\n",type);
#endif
#elif defined (CONFIG_MACH_FLIPBOOK)	
			 ...
			 const struct i2c_device_id *id)
{
	...
	if( board_hw_revision < 0x4 )
	{
				vreg_proximity = vreg_get(0, "ldo15");
				...
				vreg_set_level(vreg_proximity, OUT2600mV); // set to 3.0V voltage 
				vreg_enable(vreg_proximity); // voltage 
	}
	
#endif
	vreg_proximity_LEDA = vreg_get(0, "ldo4");

	if (IS_ERR(vreg_proximity_LEDA))
	{	
	printk("===== [PROXIMITY] proximity IS_ERR TEST =====\n");
	return PTR_ERR(vreg_proximity_LEDA);
	}

	vreg_set_level(vreg_proximity_LEDA, OUT3000mV); // set to 3.0V voltage 
	vreg_enable(vreg_proximity_LEDA); // voltage 
#elif defined (CONFIG_MACH_FLIPBOOK)	
	...
	if (i2c_smbus_read_word_data(client, 0x00) < 0)
	{
...
//choh-sysfs : create a sysfs file "proximity_int" in the switch_cmd dir
	if (device_create_file(switch_cmd_dev, &dev_attr_proximity_int) < 0)
		pr_err("Failed to create device file(%s)!\n", dev_attr_proximity_int.attr.name);
//end of choh-sysfs
