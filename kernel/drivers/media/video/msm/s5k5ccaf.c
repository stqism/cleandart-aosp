...
int s5k5ccaf_regs_table_init_for_antibanding(void);
...
void cam_pw(int status)
{
	...
	{
		PCAM_DEBUG("POWER ON");
		vreg_set_level(vreg_cam_out14_af,  OUT2800mV);
		...
		vreg_enable(vreg_cam_out14_af);			

		//gpio_set_value(1, 1); //SPECTURM IC ON   //shlee_temp

	}
	else //POWER OFF
	{
		PCAM_DEBUG("POWER OFF");
		vreg_disable(vreg_cam_out14_af);			
		...
		//gpio_set_value(1, 0); //SPECTURM IC OFF  //shlee_temp
	}
...
int s5k5ccaf_get_anti_banding(void)
{
	return s5k5ccaf_antibanding;
}

int s5k5ccaf_reg_init_60hz(void)
{
...
{
	int rc;
	printk("s5k5ccaf_regs_table_init_for_antibanding() : anti-banding ==> %d Hz\n", s5k5ccaf_get_anti_banding());
...
ssize_t s5k5ccaf_antibanding_show (struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	printk("The value of s5k5ccaf_antibanding is %d\n", s5k5ccaf_antibanding);
	return count;
}
...
ssize_t s5k5ccaf_antibanding_store (struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if ((CAM_ANTIBANDING_50HZ == (cam_antibanding_setting) tmp)
		...
		printk("The s5k5ccaf_antibanding is set to %d\n",s5k5ccaf_antibanding);
	}
...
int s5k5ccaf_sensor_init(const struct msm_camera_sensor_info *data)
{
	...
	if(rc < 0)
	{
		...
		for(i=0; i<10; i++)	// set retry count from 500 to 10 (20110308 Jeonhk)
		{
			cam_pw(0);
		...
		//  retry 10회 실패시 error return
		if(i >= 10) {
			printk("<=PCAM=> cam_hw_init time out\n");
			goto init_fail;
		}
	}
...
//latin_cam : support anti-banding
	...
	if(rv)	
		printk("<=PCAM=> s5k5ccaf_probe() : device_create_file() is failed with %d", rv);
